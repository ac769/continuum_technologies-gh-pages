...
% Find data
file_location = 'C:\Users\tm408\Documents\GitHub\continuum\software\';
fileID1 = fopen(strcat(file_location, 'data_nofilter2.txt'), 'r');
fileID2 = fopen(strcat(file_location, 'time_nofilter2.txt'), 'r');
data_ecg = fscanf(fileID1, '%f');
time = fscanf(fileID2, '%f');
...
nyq = fs/2;
highfreqcutoff = 20;
Wn = highfreqcutoff / nyq; % normalised cutoff frequency
ftype = 'low';
...
for demo = 1:2:3
    ...
    filtered1 = filter(b, a, ecg);
    %   Scale ecg
    filtered1 = filtered1(7000:end); % remove initial transience
    ...
    filtered2 = filter(b, a, filter(b, a, ecg));
    filtered2 = filtered2(7000:end); % remove initial transience
    peaks2 = filtered2/max(filtered2);
    ...
    subplot(3, 2, 3); plot(filtered1/max(filtered1));
    ...
    subplot(3, 2, 5); plot(filtered2/max(filtered2));
