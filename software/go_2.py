...
class ExampleApp(QtGui.QMainWindow, ui_main.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.ear = swhear_2.Ear(chunk=int(100))  # determines refresh rate
        # optionally you can manually set the audio input device to use like this:
        # self.ear = swhear_2.Ear(chunk=int(100), device=5) # use audio input device 5
    ...
    def update(self):
        ...
        if len(self.ear.data) and not self.btnPause.isChecked():
            ...
            if self.chkAutoscale.isChecked():
                self.Yscale = np.max(np.abs(data)) * 1.01
                # self.Yscale = 435
