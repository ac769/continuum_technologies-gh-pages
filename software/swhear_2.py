...
and rate combinations to maximize likelihood of finding one that
...
class Ear(object):
    ...
    def lowpass_filter(self, freqHighCutoff=50):
        if freqHighCutoff <= 0:
            return self.data
    ...
    def highpass_filter(self, freqLowCutoff=0.5):
        fft = np.fft.fft(self.softEdges(self.data))  # todo: filter needed?
        trim = len(fft) / self.rate * freqLowCutoff
        fft[int(trim):-int(trim)] = 0
        return np.real(np.fft.ifft(fft))

    def butter_lowpass_filter(self, freqHighCutoff=50, order=5):
    ...
    def butter_highpass_filter(self, freqLowCutoff=0.5, order=5):
    ...
    def butter_bandpass_filter(self, freqLowCutoff=0.5, freqHighCutoff=50, order=5):
